 ...
 *
 * Fonctionnalités :
 *   - Infos générales : titre, description, date limite
 *   - Membres : liste avec retrait, ajout par nom/email
 *   - Tâches : liste avec statut, priorité, assignation, édition inline, suppression
 *   - Statistiques et barre de progression
 *   - Confirmations de suppression (projet + tâche)
 */

import { useEffect, useMemo, useState } from 'react'
import TaskStatusMenu from './TaskStatusMenu'

/** Options de priorité avec libellé et classe CSS. */
...
function buildTeamList(project, globalMembers) {
  ...
  ;(project.members || []).forEach((uid) => {
    ...
    list.push({
      ...
      role: project.memberRoles?.[uid] || member?.role || 'Membre',
      pending: false,
    })
  })
...
function ProjectDetailsModal({
  ...
  // ── États ajout tâche (avec priorité et assignation) ──
  ...
  // ── États édition inline titre tâche ──
  const [editingTaskId, setEditingTaskId] = useState(null)
  const [editingTaskTitle, setEditingTaskTitle] = useState('')

  // Clé d'initialisation : change uniquement quand la modal s'ouvre sur un nouveau projet.
  // Évite de réinitialiser les états locaux à chaque notification Firestore temps réel.
  ...
  useEffect(() => {
    ...
    setEditingTaskTitle('')
  // On ne dépend que de initKey — pas de `project` entier pour éviter les re-init Firestore.
  ...
  // Membres inscrits uniquement (pas les invitations en attente) pour l'assignation.
  const assignableMembers = useMemo(
    () => teamMembers.filter((m) => !m.pending),
    [teamMembers],
  )
  ...
  const handleAddMember = async () => {
    ...
    if (!result?.ok) return
    // Mise à jour locale immédiate pour affichage instantané dans la modal.
    // Firestore va aussi mettre à jour via onSnapshot mais buildTeamList
    // déduplique déjà les UIDs donc pas de doublon.
  ...
  // ── Ajout tâche — sauvegarde immédiate dans Firestore ──
  const handleAddTask = async () => {
    const cleanTitle = newTaskTitle.trim()
    if (!cleanTitle) return

    if (onAddTask) {
      // Sauvegarde immédiate dans Firestore via le parent.
      ...
      // Fallback : brouillon local si onAddTask non fourni.
  ...
  const confirmEditTask = (task) => {
    ...
    if (isDraftTask(task.id)) {
      ...
      // Passe le statut existant + le nouveau titre au parent.
  ...
  const handleSave = (event) => {
    ...
    onSave({
      ...
      pendingEmails,
      memberRoles,
      newTasks: [], // Les tâches sont déjà sauvegardées via onAddTask.
    })
  }

  /** Retourne le nom du membre assigné à une tâche, ou null. */
  const getAssigneeName = (task) => {
    if (!task.assigneeId) return null
    const m = globalMembers.find((mb) => mb.id === task.assigneeId)
    return m?.name || null
  }

  /**
   * Change la priorité d'une tâche.
   * Pour les brouillons : mise à jour locale ; pour les tâches Firestore : délègue au parent.
   */
  const handleTaskPriorityChange = (task, newPriority) => {
    ...
    // Passe le statut existant + la nouvelle priorité au parent.
    onTaskStatusChange?.(task, task.status || 'A faire', undefined, newPriority)
  }

  /**
   * Change l'assigné d'une tâche.
   * Pour les brouillons : mise à jour locale ; pour les tâches Firestore : délègue au parent.
   */
  const handleTaskAssigneeChange = (task, newAssigneeId) => {
    if (isDraftTask(task.id)) {
      setPendingTasks((prev) =>
        prev.map((t) => (t.id === task.id ? { ...t, assigneeId: newAssigneeId || null } : t)),
      )
      return
    }
  ...
  return (
    <div className="modal-overlay" role="presentation" onClick={handleBackdropClick}>
      <div className="pdm-shell" role="dialog" aria-modal="true" aria-labelledby="pdm-title">
        ...
        <form className="pdm-body" onSubmit={handleSave}>
          ...
          <div className="pdm-col">
            ...
            <section className="pdm-section">
              <h3 className="pdm-section-title">
                <span className="pdm-section-icon" aria-hidden="true">📋</span>
                Informations
              </h3>
              <div className="pdm-fields">
                <label className="pdm-label">
                  Titre du projet
                  <input className="pdm-input" type="text" value={title} onChange={(e) => setTitle(e.target.value)} required />
                </label>
                <label className="pdm-label">
                  Description
                  <textarea className="pdm-input pdm-textarea" rows="3" value={description} onChange={(e) => setDescription(e.target.value)} placeholder="Décrivez votre projet..." />
                </label>
                <label className="pdm-label">
                  Date limite
                  <input className="pdm-input" type="date" value={deadline} onChange={(e) => setDeadline(e.target.value)} />
                </label>
              </div>
            </section>
                ...
                <ul className="pdm-member-list">
                  {teamMembers.map((m) => (
                    <li key={m.key} className="pdm-member-item">
                      ...
                      <button
                        type="button"
                        className="pdm-member-remove"
                        onClick={() => {
                          if (m.pending) {
                            setPendingEmails((prev) => prev.filter((e) => e !== m.email))
                          } else {
                            setMemberUids((prev) => prev.filter((uid) => uid !== m.uid))
                            setMemberRoles((prev) => { const next = { ...prev }; delete next[m.uid]; return next })
                          }
                        }}
            ...
            <section className="pdm-section">
              ...
              <div className="pdm-stats">
                <div className="pdm-stat-row">
                  <span className="pdm-stat-item pdm-stat-todo">
                    <span className="pdm-stat-dot" />
                    <span className="pdm-stat-num">{projectStats.todo}</span>
                    <span className="pdm-stat-label">À faire</span>
                  </span>
                  <span className="pdm-stat-item pdm-stat-progress">
                    <span className="pdm-stat-dot" />
                    <span className="pdm-stat-num">{projectStats.inProgress}</span>
                    <span className="pdm-stat-label">En cours</span>
                  </span>
                  <span className="pdm-stat-item pdm-stat-done">
                    <span className="pdm-stat-dot" />
                    <span className="pdm-stat-num">{projectStats.done}</span>
                    <span className="pdm-stat-label">Terminées</span>
                  </span>
                </div>
                <div className="pdm-progress-wrap">
                  <div className="pdm-progress-bar">
                    <div
                      ...
                      role="progressbar"
                      aria-valuenow={projectStats.completion}
                      aria-valuemin={0}
                      aria-valuemax={100}
                    />
                  </div>
                ...
                <p className="pdm-hint" style={{ marginTop: '0.35rem' }}>
                  {projectStats.done} sur {projectStats.total} tâche{projectStats.total > 1 ? 's' : ''} terminée{projectStats.done > 1 ? 's' : ''}
                </p>
              </div>
            </section>
            ...
            <section className="pdm-section pdm-tasks-section">
                    ...
                    return (
                      <div key={t.id} className="pdm-task-item">
                        <div className="pdm-task-main">
                          ...
                          {editingTaskId === t.id ? (
                            <input
                              className="pdm-input pdm-task-edit-input"
                              type="text"
                              value={editingTaskTitle}
                              onChange={(e) => setEditingTaskTitle(e.target.value)}
                              onKeyDown={(e) => {
                                if (e.key === 'Enter') { e.preventDefault(); confirmEditTask(t) }
                                if (e.key === 'Escape') cancelEditTask()
                              }}
                              onBlur={() => confirmEditTask(t)}
                              autoFocus
                          ...
                          {/* Badges priorité + assigné */}
                          <div className="pdm-task-meta">
                            {/* Sélecteur de priorité inline */}
                            <select
                              className={`pdm-priority-select ${priorityClass(t.priority || 'Moyenne')}`}
                              value={t.priority || 'Moyenne'}
                              onChange={(e) => handleTaskPriorityChange(t, e.target.value)}
                            ...
                            {/* Sélecteur d'assigné inline */}
                            <select
                              className="pdm-assignee-select"
                              value={t.assigneeId || ''}
                        ...
                        <div className="pdm-task-actions">
                          <TaskStatusMenu
                            ...
                            onChange={(next) => handleTaskStatusChange(t, next)}
                            disabled={isSubmitting || editingTaskId === t.id}
                            compact
                          />
                          {editingTaskId === t.id ? (
              ...
              <div className="pdm-new-task-form">
                ...
                <div className="pdm-new-task-options">
                  ...
                  <select
                    className="pdm-input pdm-select"
                  ...
                  <button type="button" className="button button-primary pdm-add-btn" onClick={handleAddTask}>
                    + Ajouter
                  </button>
                </div>
              </div>
          ...
          <footer className="pdm-footer">
            <button type="button" className="button button-danger-outline" onClick={() => setConfirmState({ type: 'project' })} disabled={isSubmitting}>
              🗑 Supprimer le projet
            </button>
      ...
      {confirmState?.type === 'project' && (
        <ConfirmDialog message="Supprimer ce projet ?" detail="Toutes les tâches associées seront définitivement supprimées." onConfirm={handleConfirm} onCancel={() => setConfirmState(null)} />
      )}
      {confirmState?.type === 'task' && (
        <ConfirmDialog message={`Supprimer "${confirmState.task.title}" ?`} detail="Cette tâche sera définitivement supprimée." onConfirm={handleConfirm} onCancel={() => setConfirmState(null)} />
      )}
    </div>
  )
}
